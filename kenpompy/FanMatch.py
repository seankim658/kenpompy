...
class FanMatch:
    ...
    def __init__(self, browser: CloudScraper, date: Optional[str] = None):
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to retrieve {self.url} (status code: {response.status_code})"
            )
        fm = BeautifulSoup(response.content, "html.parser")
        table = fm.find_all("table")[0]
        fm_dfs = pd.read_html(StringIO(str(table)))
        fm_df = fm_dfs[0]
        fm_df = fm_df.rename(
            columns={
                "Thrill Score": "ThrillScore",
                "Come back": "Comeback",
        ...
        fm_df["Game"] = fm_df.Game.str.replace(
            r"(\s+[A-Za-z]{2,}-T|NCAA)$", "", regex=True
        )

        pos = fm_df.Game.str.split(r" \[").str[1]
        fm_df["Game"], fm_df["Possessions"] = fm_df.Game.str.split(r" \[").str[
            0
        ], pos.astype("str")
        ...
        pred_winner = [
            " ".join(i) if not any(pd.isnull(i)) else float("nan") for i in pred_winner
        ]
        ...
        fm_df["PredictedMOV"] = [
            (int(x[0]) - int(x[1])) if len(x) > 1 else float("nan")
            for x in fm_df.PredictedScore.astype("str").str.split("-")
        ]

        fm_df[["Time", "Channel"]] = fm_df["Time (ET)"].str.extract(
            r"(\d{1,2}:\d{2} (?:am|pm))(?:\s+(.*))?"
        )

        fm_df[["Team1", "Team2", "Team1Rank", "Team2Rank", "Neutral"]] = fm_df["Game"].apply(self._parse_game)
        ...
        if not all(pd.isnull(loser)):
            ...
            fm_df["Loser"] = [
                " ".join(x.split()[1:-1]) if len(x.split(" at ")) < 2 else float("nan")
                for x in loser
            ]
            fm_df["LoserRank"] = [x.split()[0] for x in loser]
            fm_df["LoserScore"] = [
                x.split()[-1] if len(x.split(" at ")) < 2 else float("nan")
                for x in loser
            ]
        ...
        fm_df["Winner"] = [
            (
                " ".join(x.split()[1:-1])
                if (len(x.split(" at ")) < 2 and len(x.split(" vs. ")) < 2)
                else float("nan")
            )
            for x in winner
        ]
        ...
        if not all(pd.isnull(loser)):
            fm_df["ActualMOV"] = [
                (int(x[0]) - int(x[1])) if not pd.isnull(x[0]) else float("nan")
                for x in list(
                    zip(fm_df.WinnerScore.tolist(), fm_df.LoserScore.tolist())
                )
            ]
        else:
    ...
    def _parse_game(self, game_str: str) -> pd.Series:
        """Parses a game string into the teams, ranks, and neutral game status."""
        game_str = game_str.replace('\xa0', ' ').strip()
        delimiter = r"\s*(?:vs\.|at)\s*"

        space_splits = game_str.split(" ")
        space_splits = [x.strip().lower() for x in space_splits]
        neutral = True if "vs." in space_splits else False

        teams = re.split(delimiter, game_str)
        print(game_str)
        print(teams)

        if len(teams) != 2:
            return pd.Series([pd.NA, pd.NA, pd.NA, pd.NA, neutral])

        def extract_team_info(team: str):
            rank_match = re.search(r"\b(NR|\d+)\b", team)
            rank = rank_match.group(0) if rank_match else "NR"
            name = re.sub(r"\b(NR|\d+)\b", "", team).strip()
            return name, rank

        team1_name, team1_rank = extract_team_info(teams[0])
        team2_name, team2_rank = extract_team_info(teams[1])

        return pd.Series([team1_name, team2_name, team1_rank, team2_rank, neutral])
